...
from adql_analytics.transforms.radar_profile import player_dataframe_to_c04_payload


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gera um JSON C-04 Radar Profile a partir de estatísticas do FBref."
    )
    parser.add_argument(
        "--league",
    ...
    parser.add_argument(
        "--player",
        default=None,
        help='Jogador do radar. Use aspas em nomes compostos. Ex.: --player "Bukayo Saka"',
    )
    parser.add_argument(
        "--stat-types",
    ...
    parser.add_argument(
        "--output",
        default="c04_fbref_radar_profile.json",
...
def main() -> None:
    ...
    payload = player_dataframe_to_c04_payload(
        df=df,
        player=args.player,
        title=f"Perfil FBref — {player_label}",
